    ...
    try:
        ...
        cursor.execute("""
            SELECT
                ...
                COALESCE(s.display_author, u.username) as author, u.bio as author_bio,
                GROUP_CONCAT(t.name) as categories
            FROM stories s
            JOIN users u ON s.user_id = u.id
            LEFT JOIN story_tags st ON s.id = st.story_id
            ...
            GROUP BY s.id, s.title, s.content, s.description, s.language_name,
                     ...
                     s.reading_time, s.word_count,
                     s.status, s.view_count, s.like_count, s.created_at, s.updated_at, s.published_at,
                     s.display_author, u.username, u.bio
        """, (story_id,))
    ...
    try:
        ...
        cursor.execute("""
            SELECT
                ...
                s.view_count,
                s.like_count,
                COALESCE(s.display_author, u.username) as author,
                GROUP_CONCAT(t.name) as categories
            FROM stories s
            JOIN users u ON s.user_id = u.id
            LEFT JOIN story_tags st ON s.id = st.story_id
            LEFT JOIN tags t ON st.tag_id = t.id
            WHERE s.status = 'published' AND s.deleted_at IS NULL
            GROUP BY s.id, s.title, s.description, s.image_path, s.audio_path, s.video_path, s.created_at, s.view_count, s.like_count, s.display_author, u.username
            ORDER BY s.created_at DESC
        """)
    ...
    try:
        ...
        cursor.execute("""
            ...
            GROUP BY s.id, s.title, s.description, s.status, s.created_at, s.updated_at,
                     s.view_count, s.like_count, s.word_count, s.display_author, u.username, u.email
            ORDER BY s.created_at DESC
            LIMIT %s OFFSET %s
    ...
    try:
        ...
        cursor.execute("""
            ...
            GROUP BY s.id, s.title, s.content, s.description, s.status, s.created_at,
                     ...
                     s.image_path, s.image_original_name, s.display_author, u.username, u.email
        """, (story_id,))
...
# =====================================================
# Admin Story Metadata Management
# =====================================================

@app.route('/admin/api/update_story_metadata/<int:story_id>', methods=['POST'])
@admin_required
def admin_update_story_metadata(story_id):
    """Update story display metadata (author and publish time)"""
    try:
        # Get data from request
        data = request.get_json()
        display_author = data.get('display_author', '').strip()
        published_at_str = data.get('published_at', '').strip()

        # Validate inputs
        if not display_author and not published_at_str:
            return jsonify({'success': False, 'error': 'Please provide at least one field to update'}), 400
        ...
        if display_author:
            update_fields.append("display_author = %s")
            params.append(display_author)
        ...
        logger.error(f"Failed to update story metadata: {str(e)}")
    ...
    try:
        ...
        stories_query = f"""
            SELECT s.id, s.title, s.description, s.status, s.word_count, s.view_count,
                   ...
                   COALESCE(s.display_author, u.username) as author_name,
                   u.email as author_email,
                   GROUP_CONCAT(t.name) as tags
            FROM stories s
            LEFT JOIN users u ON s.user_id = u.id
            ...
            GROUP BY s.id, s.title, s.description, s.status, s.word_count, s.view_count,
                     s.created_at, s.updated_at, s.deleted_at, s.display_author, u.username, u.email
